 ...
 * nocturne.ts
 *
 * A monochrome nightscape rendered as character-art on an HTML canvas: a
 * side-lit 3D moon sampled on a monospace glyph grid, where each cell's
 * luminance maps to a glyph from a density ramp and a gray from the palette,
 * plus a sparse scatter of twinkling stars. Same idea as an image→ASCII
 * converter, but grayscale, in Geist Mono, and driven by a live moon.
 */
